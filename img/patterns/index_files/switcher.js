...
$(document).ready(function() {
	$('#color-changer-heading').styleSwitcher2();
});
...
(function($) {
	$.fn.styleSwitcher2 = function(options){		
		var defaults = {	
			slidein: true, preview: true, container: this.selector, directory: "css/colors/", useCookie: true, cookieExpires: 30, manageCookieLoad:true	
		};
		...
		if (opts.useCookie && opts.manageCookieLoad) {
			...
			if(isCookie){
				...
				$("link[id=heading]").attr("href",newStyle);
				baseStyle = newStyle;
			}
			else{
		...
		var baseStyle = $("link[id=heading]").attr("href");
		if(opts.preview){
			$(opts.container + " a").click(
				...
				function () {
					$("link[id=heading]").attr("href",baseStyle);
				}
			);
		}
...
jQuery(document).ready(function($) {
    ...
    $(".switcher").css('left', '-254px');
   	
	$(".switcher-trigger").click(function(){
	
        if ($('.switcher').hasClass('closeswitcher')){
			$('.switcher').animate({left:'-254px'}, 500, function(){
    ...
    $('#switcher-option-layout').change(function() {
        ...
        } else if( switcheropLayout == 'layout-boxed' ) {
            $('body').addClass('boxed');
        }
